        ...
        with sync_playwright() as p:
            ...
            while True:
                ...
                try:
                    ...
                    # 定义 orderBy 到中文的映射字典
                    order_by_mapping = {
                        # 将 seSpvIndex 映射为 热搜词
                        "seSpvIndex": "热搜词",
                        # 将 seSpvChangeRate 映射为 飙升词
                        "seSpvChangeRate": "飙升词",
                        # 将 supplyAndDemandIndex 映射为 蓝海词
                        "supplyAndDemandIndex": "蓝海词"
                    }
                    ...
                    # 根据映射字典转换为中文，如果不在字典中则使用原值或默认值 '未知时间'
                    date_type_cn = date_type_mapping.get(date_type_val, date_type_val or "未知时间")

                    # 判断拦截到的接口是否是 relatedWord 接口
                    if "relatedWord.json" in response.url:
                        # 设置 relatedWord 接口的文件名前缀
                        file_prefix = "related_word"
                        
                        # 从请求 URL 中使用正则表达式提取 searchWord 参数的值
                        search_word_match = re.search(r'[?&]searchWord=([^&]+)', response.url)
                        # 判断是否成功匹配到了 searchWord
                        if search_word_match:
                            # 从 urllib.parse 导入 unquote 用于 URL 解码
                            from urllib.parse import unquote
                            # 将提取到的 searchWord 进行 URL 解码转换为中文
                            search_word = unquote(search_word_match.group(1))
                        # 如果没有匹配到 searchWord
                        else:
                            # 则使用默认的 '未知搜索词'
                            search_word = "未知搜索词"
                            
                        # 拼接最终的 JSON 文件名：前缀_搜索词_时间类型.json
                        filename = f"{file_prefix}_{search_word}_{date_type_cn}.json"
                        
                    # 否则，判断拦截到的接口是否是 keywordRank 接口
                    elif "keywordRank/list.json" in response.url:
                        # 设置 keywordRank 接口的文件名前缀
                        file_prefix = "keyword_rank"
                        
                        # 从请求 URL 中使用正则表达式提取 orderBy 参数的值
                        order_by_match = re.search(r'[?&]orderBy=([^&]+)', response.url)
                        # 如果匹配成功则获取对应的值，否则使用空字符串
                        order_by_val = order_by_match.group(1) if order_by_match else ""
                        # 根据映射字典转换为中文，如果不在字典中则使用原值或默认值 '未知排序'
                        order_by_cn = order_by_mapping.get(order_by_val, order_by_val or "未知排序")
                        
                        # 初始化级联文本字符串为 '未知类目'
                        cascader_str = "未知类目"
                        # 检查是否成功获取到了级联文本内容
                        if cascader_texts:
                            # 将文本列表拼接成一个初始字符串，使用短横线分隔
                            ...
                            # 去除字符串首尾可能多余的短横线，并赋值给级联文本字符串
                            cascader_str = parsed_prefix.strip('-') or "未知类目"
                            
                        # 拼接最终的 JSON 文件名：前缀_类目_时间类型_排序类型.json
                        filename = f"{file_prefix}_{cascader_str}_{date_type_cn}_{order_by_cn}.json"
                        
                    # 如果拦截到的是其他未知的接口
                    else:
                        # 使用时间戳生成默认的文件名
                        filename = f"unknown_data_{int(datetime.now().timestamp())}.json"
...
def get_keywords(keywords_file: str) -> list: # 定义获取产品关键词的方法
    ...
    try:
        ...
        for item in data_list:
            # 尝试获取计算所需的各个指标值，如果不存在或者为 None 则默认为 0
            # 包月价/参考价 (referencePrice)
            reference_price = item.get("referencePrice", {}).get("value")
            reference_price = float(reference_price) if reference_price is not None else 0.0
            
            # 点击率 (clkRate)
            clk_rate = item.get("clkRate", {}).get("value")
            clk_rate = float(clk_rate) if clk_rate is not None else 0.0
            
            # 在线商品数 (seMaxRescnt)
            se_max_rescnt = item.get("seMaxRescnt", {}).get("value")
            se_max_rescnt = float(se_max_rescnt) if se_max_rescnt is not None else 0.0
        ...
        # 打印提示信息，准备输出全部关键词
        print(f"--- 文件 {os.path.basename(keywords_file)} 中的全部关键词 ---")
