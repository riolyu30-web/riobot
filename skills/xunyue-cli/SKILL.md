...
python {homespace}\\tools\\chromecli\\cli.py open --url http://web-2.xyzlerp.com/#/inventory/finish-return-tern
```
...
python {homespace}\\tools\\chromecli\\cli.py fill --id <页面ID> --xpath "<账号输入框XPath>" --value "<用户名>"
```
...
python {homespace}\\tools\\chromecli\\cli.py --id <页面ID> --xpath "<密码输入框XPath>" --value "<密码>"
```
...
```bash
# 执行 touch 命令点击登录按钮
python {homespace}\\tools\\chromecli\\cli.py touch --id <页面ID> --xpath "<登录按钮XPath>"
```
