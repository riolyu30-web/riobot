...
class Messages():
    ...
    @staticmethod
    def pull_messages_with_sender(friend:str,number:int,search_pages:int=None,is_maximize:bool=None,close_weixin:bool=None)->list[dict]:
        '''
        该函数用来从聊天界面获取聊天消息，并能够通过坐标和结构判断区分是"自己"发送的还是"别人"发送的。
        如果在群聊中且开启了显示群成员昵称，还能尝试获取具体的发送人昵称。
        
        Args:
            friend:好友或群聊名称
            number:获取的消息数量
            search_pages:打开好友聊天窗口时在会话列表中查找好友时滚动列表的次数
            is_maximize:微信界面是否全屏，默认不全屏
            close_weixin:任务结束后是否关闭微信，默认关闭
        Returns:
            messages: [{'sender': '自己'|'对方/群成员', 'content': '消息内容'}]
        '''
        if is_maximize is None:
            is_maximize=GlobalConfig.is_maximize
        if close_weixin is None:
        ...
        # 计算整个聊天列表的中心X坐标，用于区分左右
        chat_list_rect = chat_list.rectangle()
        center_x = chat_list_rect.mid_point().x
        
        def parse_message_item(listitem):
            content = listitem.window_text()
            rect = listitem.rectangle()
            item_center_x = rect.mid_point().x
            
            sender = "未知"
            # 自己的消息气泡通常偏右侧
            if item_center_x > center_x:
                sender = "自己"
            else:
                # 别人的消息偏左侧
                sender = friend # 单聊默认是对方
                # 尝试从UI结构中提取群成员昵称（通常在气泡上方，或者本身包含在文本开头）
                # 由于pywinauto提取的text可能包含换行，例如 "张三\n你好"
                if '\n' in content:
                    parts = content.split('\n', 1)
                    if parts[0] != content: # 说明第一行可能是昵称
                        sender = parts[0]
                        content = parts[1]
                else:
                    # 如果只有一行，群聊里可能拿不到昵称，暂时标记为对方
                    sender = friend
        ...
        # 先拿最后一条
        last_item = chat_list.children(control_type='ListItem')[-1]
        if last_item.class_name() != 'mmui::ChatItemView':
            messages.append(parse_message_item(last_item))
            
        while len(messages) < number:
            chat_list.type_keys('{UP}')
            selected = [listitem for listitem in chat_list.children(control_type='ListItem') if listitem.has_keyboard_focus()]
            if selected and selected[0].class_name() != 'mmui::ChatItemView':
                messages.append(parse_message_item(selected[0]))
            if not selected:
                break
                
        chat_list.type_keys('{END}')
        
        # 因为我们是从下往上（UP）收集的，所以要反转一下列表使其符合时间顺序
        messages = messages[::-1]
        messages = messages[-number:]
...
class Monitor():
    ...
    @staticmethod
    def patrol_new_messages(duration:str, callback, interval:float=3.0, is_maximize:bool=None, search_pages:int=None, close_weixin:bool=True):
        '''
        全局巡逻小红点模式：每隔指定时间检查微信是否有新消息，如果有，则自动读取并交给回调函数处理。
        相比于一直死守在一个聊天框，此方法更适合同时监控多个人或群聊的新消息。
        
        Args:
            duration: 巡逻总时长，格式:'s','min','h'，例如 '1h' 代表巡逻1小时
            callback: 收到新消息后的回调函数。该函数应接收两个参数：friend(发送者名称) 和 messages(新消息列表)
            interval: 每次巡逻的间隔时间（秒），默认3秒检查一次
            is_maximize: 微信界面是否全屏，默认不全屏
            search_pages: 查找好友的翻页次数
            close_weixin: 整个巡逻任务结束后是否关闭微信主窗口，默认关闭
            
        Examples:
            >>> from pyweixin import Monitor
            >>> def my_callback(friend, messages):
            >>>     print(f"收到来自 {friend} 的新消息：{messages}")
            >>>     # 在这里你可以将消息存入数据库，或者做其他处理
            >>> Monitor.patrol_new_messages(duration='30min', callback=my_callback, interval=5)
        '''
        if is_maximize is None:
            is_maximize=GlobalConfig.is_maximize
        if search_pages is None:
        ...
        total_duration = Tools.match_duration(duration)
        ...
        print(f"开始全局巡逻小红点，持续时间: {duration}，检查间隔: {interval}秒")
        SystemSettings.open_listening_mode(volume=False)
        ...
        try:
            while time.time() < end_timestamp:
                ...
                if new_message_num > 0:
                    print(f"发现 {new_message_num} 条新消息，开始扫描会话列表...")
                    # 扫描出到底是谁发来的，各发了多少条
                    new_message_dict = scan_for_new_messages(main_window=main_window, is_maximize=is_maximize, close_weixin=False)
                    
                    for friend, num in new_message_dict.items():
                        print(f"正在提取 [{friend}] 的 {num} 条新消息...")
                        # 提取最新消息，使用带发送者信息的方法
                        messages = Messages.pull_messages_with_sender(friend=friend, number=num, close_weixin=False, search_pages=search_pages)
                        
                        # 把提取到的消息交给用户的回调函数处理
                        try:
                            callback(friend, messages)
